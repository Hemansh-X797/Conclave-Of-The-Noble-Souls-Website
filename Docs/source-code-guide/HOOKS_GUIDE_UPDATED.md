...
const {
  ...
  getPathway,         // Function: (pathwayId) => object
  loading,            // Boolean
  error               // Error string or null
} = usePathways(userRoles);

// NOTE: Join/leave pathway functionality requires API implementation
// See API_GUIDE.md for implementing /api/pathways/join and /api/pathways/leave
```
